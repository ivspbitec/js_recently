...
(function($){
	...
	/*######## Assign options  ###################################################*/
			...
			_this._verbose('$.it_recently_.prototype:_init('+_this.scope_id+')');
		...
		set:function(){		
			...
			_this._verbose('$.it_recently_.prototype:set('+_this.scope_id+')');
		...
		_scope:function(id,parent){ //scope object
			...
			this.save=function(){
				this.save_data('sit_recentlys6_'+this.id,this.data);
			},		
			
			this.delete=function(){
				this.save_data('sit_recentlys6_'+id,'');
			},
			...
			var scope=this.load_data('sit_recentlys6_'+this.id);
...
//console.log(this.item_template);
			...
			for (var n in is) {			
				...
				if (k<=this.options.items_count+1)
					this.$o.append(this._item_template_parse(is[n],this.options.item_template));
